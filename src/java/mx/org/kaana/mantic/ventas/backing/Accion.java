...
public class Accion extends IBaseVenta implements Serializable {
  ...
  public void doLoad() {
	...
	protected void toLoadSucursales() {
  ...
  public String doAceptar() {  
	...
	public void doActualizaPrecioCliente() {
        ...
        ((TicketVenta)this.getAdminOrden().getOrden()).setIkCliente(clienteSeleccion);
      ...
      ((TicketVenta)this.getAdminOrden().getOrden()).setIkCliente(clienteSeleccion== null? new UISelectEntity(Constantes.VENTA_AL_PUBLICO_GENERAL_ID_KEY): clienteSeleccion);
			this.doReCalculatePreciosArticulos(precioVigente, clienteSeleccion== null? Constantes.VENTA_AL_PUBLICO_GENERAL_ID_KEY: clienteSeleccion.getKey());
      this.toLoadRegimenesFiscales();
		} // try
		catch (Exception e) {
			Error.mensaje(e);
	...
	protected void toMoveArticulo(Articulo articulo, Integer index) throws Exception {
		...
		try {
			clienteSeleccion= (UISelectEntity) this.attrs.get("clienteSeleccion");
			if(clienteSeleccion!= null && !clienteSeleccion.getKey().equals(-1L)){
				descuentoVigente= this.toDescuentoVigente(articulo.getIdArticulo(), clienteSeleccion.getKey());				
				if(descuentoVigente!= null) {
					descuentoPivote= getAdminOrden().getDescuento();
  ...
  @Override
	protected String toDescuentoVigente(Long idArticulo, Long idCliente) throws Exception {
		MotorBusqueda motorBusqueda= null;
    ...
    try {
      ...
      if(cliente!= null) {
        ...
        ((TicketVenta)this.getAdminOrden().getOrden()).setTransporta((String)this.attrs.get("transporta"));
      } // if  
    } // try
    catch (Exception e) {
      Error.mensaje(e);
