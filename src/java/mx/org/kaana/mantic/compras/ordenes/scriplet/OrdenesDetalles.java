    ...
    super.afterDetailEval();
     ...
     BitMatrix matrix         = null;
      try {
        ...
        matrix = writer.encode(getParameterValue("NOMBRE_REPORTE").toString().concat(":").concat(getFieldValue("CONSECUTIVO").toString()).toString().concat("-").concat(Configuracion.getInstance().getPropiedadServidor("sistema.dns")).concat("Control/generar.jsf?faces-redirect=true"), BarcodeFormat.QR_CODE, 400, 400, hints);
