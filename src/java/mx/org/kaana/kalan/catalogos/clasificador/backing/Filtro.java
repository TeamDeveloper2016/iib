...
@Named(value = "kalanCatalogosClasificadorFiltro")
@ViewScoped
public class Filtro extends IBaseFilter implements Serializable {
  ...
  protected void init() {
    try {
      ...
      if(!Objects.equals(JsfBase.getFlashAttribute("idGastoClasificador"), null)) {
  ...
  public void doLoad() {
    ...
    try {
      ...
      this.lazyModel = new FormatCustomLazy("TcKalanGastosClasificacionesDto", "lazy", params, columns);
	...
	private Map<String, Object> toPrepare() {
	  Map<String, Object> regresar= new HashMap<>();	
		...
		if(!Cadena.isVacio(this.attrs.get("idGastoClasificador")) && !this.attrs.get("idGastoClasificador").toString().equals("-1")) {
  		sb.append("(tc_kalan_gastos_clasificaciones.id_gasto_clasificacion=").append(this.attrs.get("idGastoClasificador")).append(") and ");
      this.attrs.put("idGastoClasificador", null);
    } // if  
  ...
  public String doAccion(String accion) {
    EAccion eaccion= null;
		try {
			...
			JsfBase.setFlashAttribute("retorno", "/Paginas/Kalan/Catalogos/Clasificador/filtro");		
			JsfBase.setFlashAttribute("idGastoClasificador", (eaccion.equals(EAccion.MODIFICAR) || eaccion.equals(EAccion.CONSULTAR)) ? ((Entity)this.attrs.get("seleccionado")).getKey() : -1L);
		} // try
		catch (Exception e) {
			Error.mensaje(e);
			JsfBase.addMessageError(e);			
		} // catch
		return "/Paginas/Kalan/Catalogos/Clasificador/accion".concat(Constantes.REDIRECIONAR);
  } // doAccion  
  ...
  public void doEliminar() {
		...
		try {
			...
			if(transaccion.ejecutar(EAccion.ELIMINAR))
				JsfBase.addMessage("Eliminar", "El clasificador se eliminó correctamente", ETipoMensaje.ERROR);
			else
				JsfBase.addMessage("Eliminar", "Ocurrió un error al eliminar el clasificador", ETipoMensaje.ERROR);								
		} // try
		catch (Exception e) {
			Error.mensaje(e);
			JsfBase.addMessageError(e);			
