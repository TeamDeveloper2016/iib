 ...
 */
@Named(value = "kajoolAccesoPerfilFiltro")
@ViewScoped
public class Filtro extends IBaseAttribute implements Serializable {
  ...
  private void validaEmpleado() {
    ...
    try {
      ...
      idPersona = privilegio.getIdPersona(autentifica.getCredenciales().getCuentaInicial());
      autentifica.getPersona().setIdPersona(idPersona);
    } // try
    catch (Exception e) {
      Error.mensaje(e);
    } // catch				
  ...
  private void crearArbolGrupos(TreeNode padre) {
    ...
    try {
      ...
      for (GrupoPerfiles grupo : this.grupos) {
        ...
        crearArbolPerfiles(treeEnc, grupo.getIdGrupo(), empleado.getIdPersona());
      } // for
    } // try
    catch (Exception e) {
      Error.mensaje(e);
  ...
  private void updateUsuarioEnLinea() throws Exception {
    ...
    usuario = usuarioEnLinea.getCuenta(JsfBase.getSession().getId(), JsfBase.getAutentifica().getPersona().getCuenta());
    if (usuario != null) {
