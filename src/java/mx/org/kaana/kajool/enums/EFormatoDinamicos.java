...
public enum EFormatoDinamicos implements IFormatosKajool {
  ...
  FECHA_HORA_LARGA("yyyyMMddhhmmss"),
  FECHA_ANIO_MES("yyyy/Mes corto"),
  MEGAS("Megas"),
  ASTERISCO("*");
  ...
  public String execute(Object value) {
    String regresar = "";
    if (value != null) {
			if(value instanceof Value && (((Value)value).getData() instanceof Timestamp || ((Value)value).getData() instanceof Date || ((Value)value).getData() instanceof Time))
				value= ((Value)value).getData();
			if(value instanceof Timestamp) {
				SimpleDateFormat pattern= new SimpleDateFormat("yyyyMMddHHmmssS");
				regresar= pattern.format((Timestamp)value);
			} // if
			else
				if(value instanceof Date) {
					SimpleDateFormat pattern= new SimpleDateFormat("yyyyMMdd");
					regresar= pattern.format((Date)value);
				} // if
				else
					if(value instanceof Time) {
						SimpleDateFormat pattern= new SimpleDateFormat("HHmmssS");
						regresar= pattern.format((Time)value);
					} // if
					else
      ...
      switch (this) {
        ...
        case FECHA_HORA_ANTERIOR:
          ...
          break;
        case FECHA_ANIO_MES:
          regresar = Fecha.formatear(Fecha.FECHA_ANIO_MES, Fecha.getFechaHora(regresar));
          break;
        case MEGAS:
