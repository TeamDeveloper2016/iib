...
public abstract class IBaseDao<T extends IBaseDto> {
  ...
  protected Long delete(Long idFuenteDato, Class dto) throws Exception {
    Session session = null;
    Transaction transaction = null;
    Long regresar = -1L;
    try {
  ...
  protected T findById(Class dto) throws Exception {
    ...
    Transaction transaction = null;
    try {
      session = SessionFactoryFacade.getInstance().getSession(getIdFuenteDato());
      transaction = session.beginTransaction();
      session.clear();
  ...
  protected Long size(Session session, Class dto, String idXml, Map<String, Object> params) throws Exception {
    ...
    try {
      ...
      total = "select count(*) count from (".concat(Cadena.toSqlCleanOrderBy(sql)).concat(") datos");
